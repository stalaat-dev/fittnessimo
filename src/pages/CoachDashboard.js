...
export default function CoachDashboard({ session }) {
          ...
          await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              ...
              html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
                ...
                <p style="color:#444;line-height:1.6">Your coach just uploaded a new session: <strong>${wTitle}</strong>.</p>
  ...
  const updateEx = (i, field, val) => {
    const n = [...exercises]; n[i][field] = val; setExercises(n)
  }
                                      ...
                                      {ws.exercises?.map((ex, i) => {
                                        ...
                                        return (
                                          <div key={i} style={s.detailExRow}>
                                            ...
                                            {ex.videoUrl && <a href={ex.videoUrl} target="_blank" rel="noreferrer" style={s.videoLink}>▶ {ex.videoLabel || 'Watch video'}</a>}
                                            {clientLog && (
        ...
        {tab === 'Build workout' && (
          <div style={{ maxWidth: 720 }}>
            ...
            <div style={s.card}>
              ...
              <textarea value={wNote} onChange={e => setWNote(e.target.value)} placeholder="General message for the client about this session…" rows={2} style={{ ...s.inputSm, resize: 'none' }} />
              ...
              {showImport && (
                <div style={s.importBox}>
                  <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: '#888', marginBottom: 8 }}>Paste your Claude-generated workout below</div>
                  <textarea
                    value={importText}
                    ...
                    rows={10}
                    style={{ ...s.exInput, width: '100%', resize: 'vertical', marginTop: 4, fontFamily: 'monospace', fontSize: 12, lineHeight: 1.6 }}
                  />
              ...
              {exercises.map((ex, i) => (
                <div key={i} style={s.exBlock}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                    <div><div style={s.exLabel}>Video label</div><input value={ex.videoLabel} onChange={e => updateEx(i, 'videoLabel', e.target.value)} placeholder="e.g. RDL tutorial" style={s.exInput} /></div>
                  ...
                  <div>
                    <div style={s.exLabel}>Coach cue (visible to client under exercise name)</div>
                    <textarea value={ex.comment} onChange={e => updateEx(i, 'comment', e.target.value)} placeholder="e.g. Drive through your heels, slow eccentric — 3 sec down" rows={2} style={{ ...s.exInput, resize: 'none', width: '100%' }} />
                  </div>
                </div>
              ))}
